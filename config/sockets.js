...
module.exports.sockets = {
  ...
  beforeConnect: async function (handshake, proceed) {
      ...
      if (typeof record !== 'undefined' && typeof record[0] !== 'undefined') {
        ...
        if (record.active === 1) {
          if (record.action === 'permaban' || record.action === 'dayban' || record.action === 'showban') {
            return proceed(`Your interactions with WWSU have been placed under review. Please contact WWSU. Please include the following reference number(s) in your email: ${references}`, false)
          }
        }
      }
    } catch (e) {
      sails.log.error(e)
      return proceed(`An internal server error occurred when attempting to authorize this websocket connection.`, false)
    }
    ...
    if (handshake.headers && handshake.headers.origin && (handshake.headers.origin.startsWith(sails.config.custom.baseUrl || `http://localhost:${sails.config.port}`) || handshake.headers.origin.startsWith(`http://130.108.128.116`))) {
      return proceed(`Origin allow`, false)
    } else {
      ...
      var record = await sails.models.hosts.findOrCreate({ host: handshake._query.host }, { host: handshake._query.host, friendlyname: handshake._query.host })

      if (!record.authorized) {
        return proceed(`The provided host is not yet authorized to connect to WWSU. Please have an administrator authorize this host.`, false)
      }
    }
