...
module.exports.bootstrap = async function (done) {
    ...
    return new Promise(async () => {
      ...
      try {
        ...
        await sails.helpers.status.change.with({
          name: "db-memory",
          label: "DB Memory",
          data: "The CRON checkDB failed. Please ensure the database is online and functional, and the credentials in config/datastores are correct.",
          status: 1,
        });
        await sails.helpers.status.change.with({
          name: "db-radiodj",
  ...
  sails.log.verbose(`BOOTSTRAP: scheduling inventory CRON.`);
    ...
    records
      ...
      .map((record) => {
        ...
        issues.push(
          ...
          }) is/are still checked out. They were supposed to be returned by ${moment(
            record.checkInDue
          ).format("LLLL")}.`
        );
      });
