...
module.exports = async function (req, res, next) {
  ...
  if (req.headers && req.headers.authorization) {
    ...
    return res
      .status(401)
      .json({
        errToken: "This endpoint requires auth/director authorization.",
      });
  }

  try {
    var authorized = jwt.verify(token, sails.config.custom.secrets.director, {
      subject: "director",
    });
    ...
    return res
      ...
      .json({
        errToken:
          "This endpoint requires auth/director authorization. The provided token is invalid or expired.",
      });
  }
};
