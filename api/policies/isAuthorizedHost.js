...
module.exports = async function (req, res, next) {
  ...
  if (req.headers && req.headers.authorization) {
    ...
    return res
      .status(401)
      .json({ errToken: "This endpoint requires auth/host authorization." });
  }

  try {
    var authorized = jwt.verify(token, sails.config.custom.secrets.host, {
      subject: "host",
    });
    ...
    return res
      ...
      .json({
        errToken:
          "This endpoint requires auth/host authorization. The provided token is invalid or expired.",
      });
  }
};
