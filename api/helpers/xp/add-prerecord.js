  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if (sails.models.meta['A'].state === 'live_prerecord') {
        ...
        await sails.models.xp.create({ dj: sails.models.meta['A'].dj, type: 'xp', subtype: 'showtime', amount: showXP, description: `Prerecord was on the air for ${showTime} minutes.` })
          .tolerate((err) => {
        ...
        var listeners = await sails.models.listeners.find({ dj: sails.models.meta['A'].dj, createdAt: { '>=': moment(sails.models.meta['A'].showStamp).toISOString(true) } }).sort('createdAt ASC')
          .tolerate((err) => {
            // Do not throw for error, but log it
            sails.log.error(err)
          })
        ...
        if (listeners && listeners.length > 0) {
          var prevTime = moment(sails.models.meta['A'].showStamp)
          var prevListeners = 0
          listenerMinutes = 0
          ...
          await sails.models.xp.create({ dj: sails.models.meta['A'].dj, type: 'xp', subtype: 'listeners', amount: listenerXP, description: `There were ${listenerMinutes} online listener minutes during the prerecord.` })
            .tolerate((err) => {
              // Do not throw for error, but log it
              sails.log.error(err)
            })
