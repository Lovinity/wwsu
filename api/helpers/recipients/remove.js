      ...
      if (typeof inputs.host === "undefined" || inputs.host === null) {
        ...
        for (var key in sails.models.recipients.sockets) {
          if (
            ...
            if (
              _.includes(sails.models.recipients.sockets[key], inputs.socket)
            ) {
              where.ID = key;
            ...
            if (hostRecord) {
              ...
              if (
                ...
                await sails.helpers.status.change.with({
                  ...
                  status: offStatus,
                  data: `Host is offline.${additionalData}`,
                });
              }
              ...
              if (hostRecord.delaySystem) {
                await sails.helpers.status.change.with({
                  name: "delay-system",
                  label: "Delay System",
                  data: `The host responsible for the delay system, ${recipient.label}, is offline.<strong>You will not be able to remotely dump audio in DJ Controls</strong> until this issue is resolved.`,
                  status: 2,
                });
              }
            }
