  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if (updateIt === 1 && typeof criteria.status !== 'undefined' && criteria.status <= 3 && (!record.status || (record.status !== criteria.status))) {
        ...
        await sails.models.logs.create({ attendanceID: sails.models.meta.memory.attendanceID, logtype: `status-${loglevel}`, loglevel: loglevel, logsubtype: sails.models.meta.memory.show, logIcon: `fas fa-exclamation-triangle`, title: `Status changed for ${criteria.label || record.label || criteria.name || record.name || `Unknown System`}`, event: `${criteria.data ? criteria.data : `Unknown Issue`}` }).fetch()
          .tolerate((err) => {
