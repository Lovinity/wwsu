module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(sails.config.custom.categories, config) && config !== `_doNotRemove`) {
          ...
          for (var cat in sails.config.custom.categories[config]) {
            if (Object.prototype.hasOwnProperty.call(sails.config.custom.categories[config], cat)) {
              var thecategory = await sails.models.category.findOne({ name: cat })
                .tolerate(() => {
                })
              if (!thecategory || thecategory === null) { continue }

              if (sails.config.custom.categories[config][cat].length <= 0) {
                thesubcategories = await sails.models.subcategory.find({ parentid: thecategory.ID })
                  .tolerate(() => {
                  })
              } else {
                thesubcategories = await sails.models.subcategory.find({ parentid: thecategory.ID, name: sails.config.custom.categories[config][cat] })
                  .tolerate(() => {
                  })
              }
              if (!thesubcategories || thesubcategories.length <= 0) { continue }
      ...
      categories = await sails.models.category.find({ name: [`Sports Openers`, `Sports Liners`, `Sports Closers`] })
        .tolerate(() => {
        })
      ...
      subcategories = await sails.models.subcategory.find({ parentid: catIDs })
        .tolerate(() => {
        })
      ...
      if (subcategories.length > 0) {
        ...
        subcategories.forEach((subcategory) => {
          if (typeof sails.config.custom.showcats[subcategory.name] === `undefined`) { sails.config.custom.showcats[subcategory.name] = { 'Show Openers': null, 'Show Returns': null, 'Show Closers': null } }
