...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if (!record) { return exits.success(false) }

      // Return false (fails rules) if the track is not enabled
      if (record.enabled !== 1) { return exits.success(false) }

      // Return false (fails rules) if the track has exceeded the play limit.
      if (record.limit_action > 0 && record.count_played >= record.play_limit) { return exits.success(false) }

      // Return false (fails rules) if the track's end date/time has passed.
      if (moment(record.end_date).isBefore() && moment(record.end_date).isAfter(`2002-01-01 00:00:01`)) { return exits.success(false) }

      // Return false (fails rules) if the current date/time is before the track's start date/time.
      if (moment(record.start_date).isAfter()) { return exits.success(false) }
      ...
      if (moment(record.date_played).isAfter(moment().subtract(rotationRules.RepeatTrackInterval, `minutes`))) { return exits.success(false) }
      if (moment(record.title_played).isAfter(moment().subtract(rotationRules.RepeatTitleInteval, `minutes`))) { return exits.success(false) }
      if (moment(record.artist_played).isAfter(moment().subtract(rotationRules.RepeatArtistInteval, `minutes`))) { return exits.success(false) }
      if (moment(record.album_played).isAfter(moment().subtract(rotationRules.RepeatAlbumInteval, `minutes`))) { return exits.success(false) }
