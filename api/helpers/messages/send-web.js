  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if (inputs.private) {
        ...
        records = await sails.models.messages.create({ status: 'active', from: `website-${theid}`, from_friendly: `Web (${inputs.nickname})`, from_IP: inputs.from_IP, to: 'DJ-private', toFriendly: 'DJ private', message: inputs.message }).fetch()
        ...
        records = await sails.models.messages.create({ status: 'active', from: `website-${theid}`, from_friendly: `Web (${inputs.nickname})`, from_IP: inputs.from_IP, to: 'DJ', toFriendly: 'DJ', message: inputs.message }).fetch()
        if (!records) { return exits.error(new Error('Internal Error: Could not save message to the database')) }
        delete records.from_IP // We do not want to broadcast IP addresses!
      }
