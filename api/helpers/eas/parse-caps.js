        ...
        .map(async (entry, index) => {
          ...
          if (typeof alert['id'] !== 'undefined' && typeof alert['cap:status'] !== 'undefined' && alert['cap:status'] === 'Actual') {
            ...
            if (moment().isBefore(moment(alert['cap:expires']))) {
              ...
              await sails.helpers.eas.addAlert(alert['id'], 'NWS', inputs.county, alert['cap:event'], alert['cap:severity'] !== `Unknown` ? alert['cap:severity'] : `Minor`, moment(alert['cap:effective']).toISOString(true), moment(alert['cap:expires']).toISOString(true), color, alert['title'])
