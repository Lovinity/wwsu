        ...
        maps = sails.config.custom.radiodjs
          ...
          .map(async (instance) => {
            ...
            if (!status || status.status !== 5) {
              ...
              await sails.models.announcements.findOrCreate({ type: 'djcontrols', title: `RadioDJ Outage (system)` }, { type: 'djcontrols', level: 'warning', title: `RadioDJ Outage (system)`, announcement: `On ${moment().toISOString(true).format('LLLL')}, the system ran into a condition where there were no operational RadioDJs. When this happens, dead air is likely. This is usually caused by a network outage on all computers or the Node server, or when there are no backup RadioDJs to take over a failure. Please check the logs for more info, and delete this announcement under admin menu -> Manage Announcements when the issue is considered resolved.`, starts: moment().toISOString(true), expires: moment({ year: 3000 }).toISOString(true) })
                .tolerate((err) => {
                  sails.log.error(err)
                })
