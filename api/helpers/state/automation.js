    ...
    try {
      ...
      (async () => {
        ...
        if (!inputs.transition) {
          await sails.helpers.meta.change.with({ genre: '', state: 'automation_on', show: '', track: '', topic: '', webchat: true, playlist: null, lastID: moment().toISOString(true), playlistPosition: -1, playlistPlayed: moment('2002-01-01').toISOString() })
      ...
      returnData.messagesWeb = await sails.models.messages.count({ to: { startsWith: 'website' }, createdAt: { '>=': moment(showStamp).toISOString(true) } })
        .tolerate((err) => {
          // Do not throw for error, but log it
          sails.log.error(err)
        })
