  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      await sails.helpers.attendance.createRecord(
        inputs.transition ? null : undefined
      );
      ...
      if (sails.models.meta.memory.state.startsWith("live_")) {
        await sails.helpers.break.executeArray(
          sails.config.custom.specialBreaks.live.end
        );
      }

      if (sails.models.meta.memory.state.startsWith("remote_")) {
        await sails.helpers.break.executeArray(
          sails.config.custom.specialBreaks.remote.end
        );
      }
      ...
      if (
        ...
        await sails.helpers.break.executeArray(
          sails.config.custom.specialBreaks.sports.end
        );
      }

      // If we are not coming out of a playlist, queue and play stuff
      if (
