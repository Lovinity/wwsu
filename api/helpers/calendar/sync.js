...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    // Function: Load client secret from JSON file
    ...
    // Function: Authorize OAuth client
    ...
    // Function: Go through OAuth authentication process
    ...
    try {
      ...
      if (events.length === 0) {
        ...
        if (toTrigger !== null && toTrigger.priority < 3) {
          await sails.helpers.playlists.start(toTrigger.event, toTrigger.type, toTrigger.description, inputs.ignoreChangingState, toTrigger.forced)
      ...
      if (events.length === 0) {
        ...
        for (var i2 = 0; i2 < events.length; i2++) {
          ...
          if (directors.indexOf(event.summary) === -1) {
            if (status > 3) { status = 3 }
            if (issues.indexOf(`Office Hours for "${event.summary}" exists on the Office Hours Google Calendar, but this director does not exist in the system. These hours were ignored. Please add this director to the system under DJ Controls -> Manage Directors. Do not add anything other than director office hours to the Office Hours calendar.`) === -1) { issues.push(`Office Hours for "${event.summary}" exists on the Office Hours Google Calendar, but this director does not exist in the system. These hours were ignored. Please add this director to the system under DJ Controls -> Manage Directors. Do not add anything other than director office hours to the Office Hours calendar.`) }
          } else {
            // Prepare data structure for event
            criteria = {
              unique: event.id,
