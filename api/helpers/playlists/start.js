        ...
        try {
            ...
            if (!Playlists.queuing && (((Meta['A'].changingState === null || inputs.ignoreChangingState) && ((Meta['A'].state === 'automation_on' || Meta['A'].state === 'automation_playlist' || Meta['A'].state === 'automation_genre')))))
            {
                ...
                if (inputs.type === 0)
                {
                    ...
                    await Logs.create({attendanceID: Meta['A'].attendanceID, logtype: 'primary', loglevel: 'success', logsubtype: 'playlist - ' + theplaylist.name, event: '<strong>Playlist started.</strong><br />Playlist: ' + inputs.name}).fetch()
                            .tolerate((err) => {
                                sails.log.error(err);
                            });
                    await loadPlaylist();
