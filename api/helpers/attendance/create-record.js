module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
          ...
          if (_.difference(onAirDJs, scheduledDJs).length > 0) {
            await sails.models.logs
              .create({
                ...
                logIcon:
                  sails.models.calendar.calendardb.getIconClass(nowEvent),
      ...
      if (currentID !== null) {
        // Add actualEnd
        var _currentRecord = await sails.models.attendance.updateOne(
          { ID: currentID },
...
<p>*Online listener time only includes those listening to the WWSU internet radio stream, and is calculated per-listener, per-minute. For example, 5 online listeners tuned in for an entire 30 minute show is (5*30) 2 hours, 30 minutes listener time.</p>`,
              false,
              true
            );
          })(_currentRecord);
