module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if (records.length <= 0) { return exits.success([]) }
        ...
        if (final.length > 0) {
          final.map((sched) => {
            ...
            if (!reverse) {
              // -----
              //   -----
              if (start <= sched.start && end > sched.start) { available = false }
              ...
              //   -----
              //  -------
              if (start >= sched.start && start < sched.end) { available = false }
              ...
              // -----
              // -----
              if (start <= sched.start && end >= sched.end) { available = false }
            } else {
              if (start > sched.start) { available = false }
              if (start < sched.end) { available = false }
              if (end < start) { available = false }
              if (end > sched.start) { available = false }
            }
          })
        }
              ...
              while (typeof show.proposal[0] !== `undefined` && typeof show.proposal[0].start !== `undefined`) {
                ...
                if (isAvailable(proposal.start, proposal.end)) {
                  ...
                  (async (showB, proposalB) => {
                    await sails.models.planner.update({ ID: showB.ID }, { actual: { start: proposalB.start, end: proposalB.end } }).fetch()
                  })(show, proposal)
                }
      ...
      setTimeout(async () => {
        return exits.success({ schedule: await sails.models.planner.find(), failed: badRecords })
      }, 3000)
    } catch (e) {
