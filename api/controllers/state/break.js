  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if (
        ...
        this.req.payload.lockToDJ !== null &&
        this.req.payload.lockToDJ !== sails.models.meta.memory.dj &&
        this.req.payload.lockToDJ !== sails.models.meta.memory.cohostDJ1 &&
        this.req.payload.lockToDJ !== sails.models.meta.memory.cohostDJ2 &&
        ...
        if (
          ...
          (!this.req.payload.makeCalls && !this.req.payload.answerCalls)
        ) {
          throw "forbidden";
        }
      }
      ...
      (async () => {
        await sails.models.logs
          .create({
            ...
            title: `Host requested to take a break.`,
            event: "",
          })
          .fetch()
          .tolerate((err) => {
      ...
      if (inputs.halftime) {
        ...
        await sails.helpers.break.executeArray(
          sails.config.custom.specialBreaks.sports.before
        );
        await sails.helpers.break.executeArray(
        ...
        if (
          ...
          moment().diff(moment(sails.models.status.errorCheck.prevID)) >
            60 * 50 * 1000
        ) {
          await sails.helpers.songs.queue(
            sails.config.custom.subcats.IDs,
        ...
        switch (sails.models.meta.memory.state) {
          case "live_on":
            await sails.helpers.break.executeArray(
              sails.config.custom.specialBreaks.live.before
            );
            await sails.helpers.break.executeArray(
          ...
          case "remote_on":
            await sails.helpers.break.executeArray(
              sails.config.custom.specialBreaks.remote.before
            );
            await sails.helpers.break.executeArray(
