module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if (!sails.models.meta['A'].state.startsWith('sports') && !sails.models.meta['A'].state.startsWith('automation_')) { return exits.error(new Error('Cannot execute state/sports-remote unless in automation or sports mode. Please go to automation first.')) }
      ...
      if (!sails.models.meta['A'].state.startsWith('sports')) {
        ...
        // Change meta
        sails.models.meta.changeMeta({ queueFinish: moment().add(queueLength, 'seconds').toISOString(true), state: 'automation_sportsremote', show: inputs.sport, topic: inputs.topic, trackStamp: null, lastID: moment().toISOString(true), webchat: inputs.webchat, djcontrols: this.req.payload.host })
      } else {
