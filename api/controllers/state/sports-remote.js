  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if (!sails.models.meta.memory.state.startsWith('sports')) {
        ...
        // Change meta
        await sails.helpers.meta.change.with({ queueFinish: moment().add(queueLength, 'seconds').toISOString(true), state: 'automation_sportsremote', show: inputs.sport, topic: inputs.topic, trackStamp: null, lastID: moment().toISOString(true), webchat: inputs.webchat })
      } else {
        // Otherwise, just update metadata but do not do anything else
        await sails.helpers.meta.change.with({ show: inputs.sport, topic: inputs.topic, trackStamp: null, webchat: inputs.webchat })
      }
