/* global sails, Meta, Logs, moment */

module.exports = {

    friendlyName: 'parallel',

    description: 'parallel test.',

    inputs: {

    },

    fn: async function (inputs, exits) {
        var output = ``;
        var func1 = new Promise(async (resolve, reject) => {
            output += `Started func1... `;
            await Logs.create({attendanceID: Meta['A'].attendanceID, logtype: 'test', loglevel: 'primary', logsubtype: '', event: 'This is a test log called from parallel.js'})
                    .tolerate((err) => {
                        sails.log.error(err);
                    });
            output += `FINISHED func1... `;
            return resolve();
        });
        var func2 = new Promise(async (resolve, reject) => {
            output += `Started func2... `;
            var queue = await sails.helpers.rest.getQueue();
            var stuff = [];
            queue.forEach(function (test) {
                stuff.push(test.ID);
            });
            output += `FINISHED func2... `;
            return resolve();
        });

        var func3 = new Promise(async (resolve, reject) => {
            output += `Started func3... `;
            var addition = 0;
            for (var i = 0; i < 1000000; i++)
            {
                addition += addition + 2;
            }
            output += `FINISHED func3... `;
            return resolve();
        });

        var func4 = new Promise(async (resolve, reject) => {
            output += `Started func4... `;
            var addition = 0;
            for (var i = 0; i < 1000; i++)
            {
                addition += moment().value();
            }
            output += `FINISHED func4... `;
            return resolve();
        });

        await Promise.all([func1, func2, func3, func4])
                .then((values) => {
                    return exits.success(output);
                })
                .catch((err) => {
                    return exits.error(err);
                });

    }


};
