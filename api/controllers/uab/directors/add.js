...
module.exports = {
    ...
    fn: async function (inputs, exits) {
        ...
        try {
            await Uabdirectors.create({name: inputs.name, login: bcrypt.hashSync(inputs.login, 10), admin: inputs.admin, position: inputs.position, present: false, since: moment().toISOString()}).fetch();
