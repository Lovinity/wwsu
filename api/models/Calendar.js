            ...
            try {
                ...
                if (events.length === 0) {
                    ...
                    for (var i = 0; i < events.length; i++) {
                        ...
                        theEvent = await Calendar.findOrCreate({ unique: event.id }, criteriaB);
                    ...
                    if (toTrigger !== null && toTrigger.priority < 3) {
                        ...
                        try {
                            ...
                            if (Meta['A'].state === `automation_genre`) { await sails.helpers.genre.start('Default', ignoreChangingState); }
                        }
                    }
                    ...
                    destroyed = await Calendar.update({ active: { '>=': 1 }, end: { '<=': moment().toISOString(true) } }, { active: 0 })
                        .tolerate(() => {
                        })
                        .fetch();
                    ...
                    if (events.length > 0) {
                        ...
                        cancelled = await Calendar.update({ unique: { 'nin': eventIds }, active: { '>=': 1 } }, { active: -1 })
                            .tolerate(() => {
                            })
                            .fetch();
                        ...
                        if (cancelled.length > 0) {
                            maps = cancelled.map(async (cEvent) => {
                                ...
                                if (dj !== null) { dj = await Djs.findOrCreate({ name: dj }, { name: dj, lastSeen: moment('2002-01-01').toISOString(true) }); }
                ...
                if (events.length === 0) {
                    ...
                    for (var i2 = 0; i2 < events.length; i2++) {
                        ...
                        if (directors.indexOf(event.summary) === -1 && issues.indexOf(`Office Hours for "${event.summary}" exists on Google Calendar, but this director does not exist in the system. These hours were ignored.`) === -1) {
                            ...
                            // Find existing record of event. If does not exist, create it in the Calendar.
                            theEvent = await Directorhours.findOrCreate({ unique: event.id }, criteriaB);
                            theEvent2 = await Timesheet.count({ unique: criteria.unique });
                            if (theEvent2 === 0) { theEvent2 = await Timesheet.create({ unique: criteria.unique, name: criteria.director, scheduled_in: moment(criteria.start).toISOString(true), scheduled_out: moment(criteria.end).toISOString(true), approved: 1 }).fetch(); }
                            ...
                            for (var key2 in theEvent) {
                                if (theEvent.hasOwnProperty(key2)) {
                                ...
                                try {
                                    ...
                                    if (record.length > 0) {
                                        ...
                                        if (record[0].unique === null) {
                                            ...
                                            if (record2.length > 0) { await Timesheet.destroy({ ID: record2[0].ID }).fetch(); }
                    ...
                    // Update entries in the calendar which passed their end time
                    destroyed = await Directorhours.update({ active: { '>=': 1 }, end: { '<=': moment().toISOString(true) } }, { active: 0 })
                        .tolerate(() => {
                        })
                        .fetch();
                    ...
                    if (events.length > 0) {
                        ...
                        // Entries no longer in Google Calendar should be updated to active = -1 to indicate they were canceled.
                        cancelled = await Directorhours.update({ unique: { 'nin': eventIds }, active: { '>=': 1 } }, { active: -1 })
                            .tolerate(() => {
                            })
                            .fetch();
