/* global moment, ProgressBar, io, Infinity */
...
function updateData(variable, data)
{
    ...
    variable[0](variable, data, method);
}

// Mark if a director is present or not whenever an updateData is called for the Directors
...
Calendar.push(function (data)
{
    Calendar = data;

    // Re-order events by start time
    ...
    // Update our formatted calendar variable with the new data
    ...
    Calendar.forEach(function (event, index)
    {
        if (index === 0)
            return null;
...
Eas.push(function (data, change, method)
{
    // Check for new alerts. Also check for extreme alerts.
    easExtreme = false;

    if (change.constructor === Array)
    {
        var ids = [];
        Eas.forEach(function (alert, index) {
            if (index === 0)
                return null;
            ids.push(alert.ID);
...
function doEas()
{
    ...
    } else if (easExtreme && !easActive)
    {
        ...
        Eas.forEach(function (dodo) {

            // Do not display non-extreme alerts
            if (typeof dodo['severity'] !== 'undefined' && dodo['severity'] !== 'Extreme')
                return null;
...
<h1 style="text-align: center; font-size: 3em; color: #FFFFFF">Request Line: <span class="text-warning">937-775-5555</span></h1>
        ...
        3: {name: 'Directors', class: 'info', do: true, function: function () {
            ...
            <div style="overflow-y: hidden;" class="d-flex flex-wrap" id="directors"></div></div>`;
                    ...
                    Directors.forEach(function (dodo, index) {
                        if (index === 0)
                            return null;
        ...
        4: {name: 'Events Today', class: 'success', do: true, function: function () {
            ...
            <div style="overflow-y: hidden;" class="d-flex flex-wrap" id="events"></div></div>`;
                    ...
                    if (typeof calendar[0][`Today ${moment(Meta.time).format('MM/DD')}`] !== 'undefined')
                    {
                        ...
                        {
                            calendar[0][`Today ${moment(Meta.time).format('MM/DD')}`].forEach(function (dodo) {
                                ...
                                {
                                    timeleft = `Starts ${moment(dodo.start).toNow()}`;
                                    alpha = 0.3;
                                    borderclass = 'warning';
                                } else if (moment(Meta.time).isAfter(moment(dodo.end)))
                                {
                                    timeleft = `Ended ${moment(dodo.ends).fromNow()}`;
                                    alpha = 0.1;
                                    borderclass = 'danger';
                                } else {
                                    timeleft = `Ends ${moment(dodo.ends).toNow()}`;
                                    alpha = 0.5;
                                    borderclass = 'success';
                                }
                                color = `rgba(${color.red}, ${color.green}, ${color.blue}, ${alpha});`;
        ...
        5: {name: 'Days 2-4', class: 'success', do: true, function: function () {
                $('#slide').animateCss('fadeOutUp', function () {
                    content.innerHTML = `<div class="animated fadeInDown">
                                ...
                                <th scope="col" width="32%" id="events-rowh-col1" style="border-style: none;">${moment(Meta.time).add(1, 'days').format('dddd MM/DD')}</th>
                                <th scope="col" width="32%" id="events-rowh-col2" style="border-style: none;">${moment(Meta.time).add(2, 'days').format('dddd MM/DD')}</th>
                                <th scope="col" width="32%" id="events-rowh-col3" style="border-style: none;">${moment(Meta.time).add(3, 'days').format('dddd MM/DD')}</th>
                                </tr>
                                </thead>
        ...
        6: {name: 'Days 5-7', class: 'success', do: true, function() {
                $('#slide').animateCss('fadeOutUp', function () {
                    content.innerHTML = `<div class="animated fadeInDown">
                                ...
                                <thead>
                                <tr style="border-style: none;">
                                <th scope="col" width="32%" id="events-rowh-col1" style="border-style: none;">${moment(Meta.time).add(4, 'days').format('dddd MM/DD')}</th>
                                <th scope="col" width="32%" id="events-rowh-col2" style="border-style: none;">${moment(Meta.time).add(5, 'days').format('dddd MM/DD')}</th>
                                <th scope="col" width="32%" id="events-rowh-col3" style="border-style: none;">${moment(Meta.time).add(6, 'days').format('dddd MM/DD')}</th>
                                </tr>
                                </thead>
                                <tbody id="events-body">
                                </tbody>
        ...
        102: {name: 'Weather Alerts', class: 'danger', do: false, function() {
            ...
            <div style="overflow-y: hidden;" class="d-flex flex-wrap" id="alerts"></div></div>`;
                    var innercontent = document.getElementById('alerts');
                    Eas.forEach(function (dodo, index) {
                        if (index === 0)
                            return null;
                        ...
                        {
                            timeleft = `Effective ${moment(dodo.starts).toNow()}`;
                        } else if (moment(Meta.time).isAfter(moment(dodo.expires)))
                        {
                            timeleft = `Expired ${moment(dodo.expires).fromNow()}`;
                        } else {
                            timeleft = `Expires ${moment(dodo.ends).toNow()}`;
                        }
                        color = `rgba(${color.red}, ${color.green}, ${color.blue}, ${alpha});`;
                        innercontent.innerHTML += `<div style="width: 32%; background-color: ${color};" class="d-flex align-items-stretch m-1 text-white border border-${borderclass}">
                        <div class="m-1" style="text-align: center; width: 100%"><span style="font-size: 1.5em;">${(typeof dodo['alert'] !== 'undefined') ? dodo['alert'] : 'Unknown Alert'}</span><br />
