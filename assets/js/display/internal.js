...
(function () {
    ...
    return e
            ...
            io.socket.on('orders', function (data) {
                tasks = merge(tasks, data);
                for (var key3 in tasks) {
                    if (tasks.hasOwnProperty(key3)) {
                        if (!(key3 in data))
                        {
                            delete tasks[key3];
                        } else {
                            for (var key2 in tasks[key3]) {
                                if (typeof tasks[key3] != 'undefined' && tasks[key3].hasOwnProperty(key2)) {
                                    if (!(key2 in data[key3]))
                                    {
                                        delete tasks[key3][key2];
                                    } else {
                                        for (var key in tasks[key3][key2]) {
                                            if (typeof tasks[key3][key2] != 'undefined' && tasks[key3][key2].hasOwnProperty(key)) {
            ...
            function ordersSocket()
            {
                ...
                io.socket.post('/tasks/gettasks', {}, function serverResponded(body, JWR) {
                    try {
            ...
            function metaSocket()
            {
                ...
                io.socket.post('/statemeta/getmeta', {displaysign: 'WWSU-internal'}, function serverResponded(body, JWR) {
                    try {
                        processMeta(JSON.parse(body));
                    } catch (e) {
                        console.log('FAILED CONNECTION');
            ...
            {
                console.log('attempting meta socket');
                io.socket.post('/statesystem/getstatus', {displaysign: 'WWSU-internal'}, function serverResponded(body, JWR) {
                    try {
                        processStatus(JSON.parse(body));
                    } catch (e) {
                        console.log('FAILED CONNECTION');
                    ...
                    try {
                        for (var key in response)
                        {
                            if (response.hasOwnProperty(key)) {
                                if (key.startsWith("djcontrols") && key != 'djcontrols-WIN-C5S1UBO0O18')
                                {
                                    delete response[key];
                                } else if (key == 'djcontrols-WIN-C5S1UBO0O18') {
                                    response[key].label = 'DJ Controls 018A';
                                }
                                if (key == 'radiodj-http://130.108.140.59:8080')
                                    response[key].label = 'RadioDJ 018D';
                                if (key == 'radiodj-http://130.108.140.68:8080')
                                    response[key].label = 'RadioDJ 018A';
                            }
                        }
                        statuses = merge(statuses, response);
                        ...
                        for (var key in statuses)
                        {
                            if (statuses.hasOwnProperty(key)) {
                                switch (statuses[key].status)
                                {
                                    case 1:
                                        statusBadges.innerHTML += `<span class="m-1 btn btn-danger btn-sm">${statuses[key].label}</span>`;
                                        break;
                                    case 2:
                                        statusBadges.innerHTML += `<span class="m-1 btn btn-danger btn-sm" style="background: #FF5722;">${statuses[key].label}</span>`;
                                        break;
                                    case 3:
                                        statusBadges.innerHTML += `<span class="m-1 btn btn-warning btn-sm">${statuses[key].label}</span>`;
                                        break;
                                    case 4:
                                        statusBadges.innerHTML += `<span class="m-1 btn btn-outline-success btn-sm">${statuses[key].label}</span>`;
                                        break;
                                    case 5:
                                        statusBadges.innerHTML += `<span class="m-1 btn btn-success btn-sm">${statuses[key].label}</span>`;
                                        break;
                                    default:
                                        statusBadges.innerHTML += `<span class="m-1 btn btn-outline-secondary btn-sm">${statuses[key].label}</span>`;
                                }
                            }
                        }
                    } catch (e) {
...
<h1 style="text-align: center; font-size: 3em; color: #FFFFFF">Request Line: <span class="text-warning">937-775-5555</span></h1>
            ...
            `;
                ...
                var projects = 0;
                console.log('Recalculating project slides')
                for (var key3 in tasks) {
                    if (tasks.hasOwnProperty(key3)) {
                        for (var key2 in tasks[key3]) {
                            if (typeof tasks[key3] != 'undefined' && tasks[key3].hasOwnProperty(key2)) {
                                if (Object.keys(tasks[key3][key2]).length > 0)
                                {
                                    projects++;
                                    ...
                                    <h1 style="text-align: center; font-size: 3em; color: #FFFFFF; width: 100%">Work Orders - ${key2}</h1>
                                    ...
                                    for (var key in tasks[key3][key2]) {
                                        if (typeof tasks[key3][key2] != 'undefined' && tasks[key3][key2].hasOwnProperty(key)) {
                                            ...
                                            var dodo = tasks[key3][key2][key];
                                            if (typeof dodo.priority == 'undefined')
                                                dodo.priority = 'undefined';
                                            if (dodo.priority.includes("High"))
                                            {
                                            ...
                                            {
                                                sortvalue += moment(dodo.due).valueOf();
                                            } else {
                                                sortvalue += 100000000000000;
                                            }
                                            tasksP[sortvalue] = dodo;
                                        }
                                    }
                                    var tasksO = {};
                                    ...
                                    console.log('Final slide preparating')
...
<h1 style="text-align: center; font-size: 3em; color: #FFFFFF">Request Line: 937-775-5555</h1>
        ...
        }, {"deepmerge": 2, "moment": 3}], 2: [function (require, module, exports) {
