...
function doRequests () {
  DJReq.request({ db: DJs.db(), method: 'POST', url: '/djs/get-web', data: {} }, (response) => {
    if (typeof response.stats === 'undefined') {
      ...
      analyticsTable.push([ '<i class="fas fa-gem" width="32"></i>', 'Remote Credits <i class="fas fa-question-circle fa-sm" style="padding-left: 1em;" onclick="creditsHelp()"></i>', formatInt(response.stats.semester.remoteCredits), formatInt(response.stats.overall.remoteCredits) ])

      // Reputation
      analyticsTable.push([ '<i class="fas fa-smile"></i>', 'Reputation / Responsibility <i class="fas fa-question-circle fa-sm" style="padding-left: 1em;" onclick="reputationHelp()"></i>', `<div class="progress progress-sm mr-2">
