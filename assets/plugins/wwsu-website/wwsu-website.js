...
// Define data
var Meta = { time: moment().toISOString(true), history: [], webchat: true, state: 'unknown', line1: `WWSU`, line2: `106.9 FM` }
...
// Define workers
var calendarWorker = new Worker('/wp-content/themes/mesh-child/assets/js/workers/calendar.js')
var directorsWorker = new Worker('/wp-content/themes/mesh-child/assets/js/workers/directors.js')
...
waitFor(() => {
  ...
  waitFor(() => {
    return (typeof socket !== 'undefined' && typeof socket._raw !== 'undefined')
  }, () => {
...
function doMeta (response) {
  ...
  if ('webchat' in response && !response.webchat) {
    blocked = true
    if (document.querySelector('#messages-sendmessage') !== null) { document.querySelector('#messages-sendmessage').disabled = true }
    if (document.querySelector('#messages-sendmessagep') !== null) { document.querySelector('#messages-sendmessagep').disabled = true }
    if (onlineSocketDone && document.querySelector('#messages-status') !== null) { document.querySelector('#messages-status').innerHTML = `<div class="p-3 bs-callout bs-callout-danger shadow-4 text-light"><strong>The host of the current show, or a director, has disabled the chat.</strong> The chat might be re-enabled after this show ends.</div>` }
  } else if (Meta.webchat) {
    blocked = false
    if (document.querySelector('#messages-sendmessage') !== null) { document.querySelector('#messages-sendmessage').disabled = false }
    if (document.querySelector('#messages-sendmessagep') !== null) { document.querySelector('#messages-sendmessagep').disabled = false }
  }
  ...
  if ('state' in response) {
    if (response.state.includes('automation_') || Meta.state === 'unknown') {
      if (automationpost !== 'automation') {
        if (onlineSocketDone && document.querySelector('#messages-status') !== null) { document.querySelector('#messages-status').innerHTML = `<div class="p-3 bs-callout bs-callout-default shadow-4 text-light"><strong>No one is on the air at this time.</strong> There might not be anyone in the studio at this time to read your message.</div>` }
      ...
      if (automationpost !== response.live) {
        if (onlineSocketDone && document.querySelector('#messages-status') !== null) { document.querySelector('#messages-status').innerHTML = `<div class="p-3 bs-callout bs-callout-warning shadow-4 text-light"><strong>The current show airing is prerecorded.</strong> There might not be anyone in the studio at this time to read your message.</div>` }
      ...
      if (automationpost !== response.live) {
        ...
        if (temp) { temp.remove() }
        if (onlineSocketDone && document.querySelector('#messages-status') !== null) { document.querySelector('#messages-status').innerHTML = `<div class="p-3 bs-callout bs-callout-success shadow-4 text-light"><strong>The show airing now is live.</strong> Your messages should be received by the DJ / host.</div>` }
        automationpost = response.live
      }
    }
  }
...
function updateCalendar () {
  ...
  if (temp !== null && temp2 !== null && temp3 !== null && temp3.innerHTML === `Loading...`) {
    if (typeof calendar[`${temp2.innerHTML} - ${temp.innerHTML}`] !== `undefined`) {
      temp3.innerHTML = ``
      calendar[`${temp2.innerHTML} - ${temp.innerHTML}`].map((event) => {
        ...
        if (event.active >= 1 && notificationsSupported && (device !== null || isMobile)) {
          var subscribed = Subscriptions([{ type: `calendar-once`, subtype: event.id }, { type: `calendar-all`, subtype: `${temp2.innerHTML} - ${temp.innerHTML}` }]).get().length
          if (subscribed === 0) {
      ...
      if (!notificationsSupported) {
        ...
        if (device === null && !isMobile) {
          ...
          if (subscribe !== 0) {
            temp3.innerHTML += `<p style="font-size: 1.5em;">Want push notifications whenever this show goes on the air? <a class="btn btn-primary" onclick="subscribe('calendar-all', '${escapeHTML(temp2.innerHTML)} - ${escapeHTML(temp.innerHTML)}')">Subscribe</a></p>`
          } else {
            temp3.innerHTML += `<p style="font-size: 1.5em;">Tired of push notifications? <a class="btn btn-primary" onclick="unsubscribe('${event.id}', '${escapeHTML(temp2.innerHTML)} - ${escapeHTML(temp.innerHTML)}')">Un-subscribe from all dates</a></p>`
          }
        }
      }
    } else {
...
function updateDirectors () {
  ...
  if (temp !== null && temp2 !== null && temp2.innerHTML === `Loading...`) {
    if (typeof directors[temp.innerHTML] !== `undefined`) {
      temp2.innerHTML = `<h3>${directors[temp.innerHTML].present ? `Director is <strong>in the office at WWSU</strong> right now.` : `Director is <strong>out of the office</strong> right now.`}</h3>`
...
function processDirectors () {
  try {
    directorsWorker.postMessage([Directors.db().get(), Directorhours.db().get(), moment(Meta.time).toISOString(true)])
  } catch (e) {
    console.error(e)
  }
}
        ...
        OneSignal.push(() => {
          try {
            ...
            iziToast.show({
              ...
              message: 'There was an error with OneSignal. Subscriptions and push notifications will not work at this time on the website.',
              color: 'red',
              zindex: 100,
              layout: 2,
              closeOnClick: true,
...
function sendMessage (privateMsg) {
  if (blocked || document.querySelector('#messages-nickname') === null || !quill) { return null }
  var message = quillGetHTML(quill.getContents())
  socket.post('/messages/send-web', { message: message, nickname: document.querySelector('#messages-nickname').value, private: privateMsg }, function serverResponded (response) {
    try {
...
// eslint-disable-next-line no-unused-vars
function clearChat () {
  if (document.querySelector('#messages') === null) { document.querySelector('#messages').innerHTML = '' }
}

function addMessage (data, firsttime = false) {
  var messageBox = document.querySelector('#messages')
  var shouldScroll = false
  if (messageBox !== null) { shouldScroll = messageBox.scrollTop + messageBox.clientHeight === messageBox.scrollHeight }

  messageIDs.push(data.ID)

  var position = data.fromFriendly !== (document.querySelector('#messages-nickname') !== null ? `Web (${document.querySelector('#messages-nickname').value})` : '') ? 'left' : 'right'
  var ago = moment(data.createdAt).format('hh:mm A')
  var from = `from-website`
  if (!data.from.startsWith('website')) {
    if (data.to_friendly === (document.querySelector('#messages-nickname') !== null ? `Web (${document.querySelector('#messages-nickname').value})` : '')) {
      from = `from-dj-you`
    } else {
  ...
  <div class="answer ${position}">
                ...
                <div class="name">${data.fromFriendly}${data.to === `DJ-private` || data.to.startsWith('website-') ? ' (Private Message)' : ''}</div>
                <div class="text ${from}">
                  ${data.message}
                </div>
